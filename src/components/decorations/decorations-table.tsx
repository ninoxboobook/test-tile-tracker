  ...
  if (!decorations.length) {
    return (
      <div className="text-center py-12">
        <h3 className="mt-2 text-sm font-medium text-gray-900">No decorations</h3>
        <p className="mt-1 text-sm text-gray-500">Get started by creating a new decoration.</p>
        <div className="mt-6">
          <Link
                ...
                {decorations.map((decoration) => (
                  <tr key={decoration.id}>
                    <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                      <Link href={`/decorations/${decoration.id}`} className="text-clay-600 hover:text-clay-900">
                        {decoration.name}
                      </Link>
                    </td>
                    ...
                    <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                      <Link href={`/decorations/${decoration.id}/edit`} className="text-clay-600 hover:text-clay-900">
                        Edit<span className="sr-only">, {decoration.name}</span>
                      </Link>
                    </td>
                  </tr>
                ))}
